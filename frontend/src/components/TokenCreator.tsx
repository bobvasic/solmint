...
import { TokenIdeaGenerator } from './TokenIdeaGenerator';

export const TokenCreator = () => {
    const { publicKey } = useWallet();

    // All the state variables from before remain the same...
    ...
    const [creationResult, setCreationResult] = useState<{success: boolean, message: string, details?: any} | null>(null);

    // All the handler functions (handleFileChange, onDrop, etc.) also remain the same...
    const handleFileChange = (file: File | null) => { if (file && file.type.startsWith('image/')) { setTokenImage(file); const reader = new FileReader(); reader.onloadend = () => { setImagePreview(reader.result as string); }; reader.readAsDataURL(file); } };
    const onDrop = useCallback((e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); e.stopPropagation(); if (e.dataTransfer.files && e.dataTransfer.files[0]) { handleFileChange(e.dataTransfer.files[0]); } }, []);
    const onDragOver = useCallback((e: React.DragEvent<HTMLDivElement>) => { e.preventDefault(); e.stopPropagation(); }, []);
    const onFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => { if (e.target.files && e.target.files[0]) { handleFileChange(e.target.files[0]); } };
    const cost = useMemo(() => { const serviceFee = 0.1; const baseNetworkFee = 0.002; let optionsCost = 0; if (revokeFreeze) optionsCost += 0.1; if (revokeMint) optionsCost += 0.1; if (revokeUpdate) optionsCost += 0.1; if (customCreatorInfo) optionsCost += 0.1; return serviceFee + baseNetworkFee + optionsCost; }, [revokeFreeze, revokeMint, revokeUpdate, customCreatorInfo]);


    // --- NEW: The updated handleCreateToken function ---
    const handleCreateToken = async () => {
        ...
        // We can also append other data like revoke flags
        formData.append('revokeFreeze', String(revokeFreeze));
        formData.append('walletAddress', publicKey.toBase58());

        try {
            const response = await axios.post('http://127.0.0.1:8080/api/create-token', formData, {
                headers: { 'Content-Type': 'multipart/form-data' }
            });
            setCreationResult({ success: true, message: response.data });
        } catch (err: any) {
            console.error("Token creation failed:", err);
            setCreationResult({ success: false, message: "An error occurred during token creation.", details: err.response?.data || err.message });
        } finally {
            setIsCreating(false);
        }
    };

    // The entire JSX return block remains the same as before.
    // The only change is that the button's onClick now calls the new handleCreateToken.
    return (
        <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-12 bg-gray-900 bg-opacity-50 rounded-2xl shadow-2xl p-8 backdrop-filter backdrop-blur-md border border-gray-800">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="md:col-span-2">
                        ...
                        <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-700 border-dashed rounded-md cursor-pointer" onDrop={onDrop} onDragOver={onDragOver} onClick={() => document.getElementById('file-upload')?.click()}>
                            ...
                            {imagePreview ? ( <img src={imagePreview} alt="Token preview" className="h-24 w-24 object-cover rounded-full" /> ) : ( <div className="space-y-1 text-center"> <svg className="mx-auto h-12 w-12 text-gray-500" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true"><path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg> <p className="pl-1">Drag & drop, or click to upload</p> <p className="text-xs text-gray-600">PNG, JPG, GIF up to 1MB</p> </div> )}
                        </div>
                    </div>
                    <div className="md:col-span-2">
                        ...
                        <div className="p-6 bg-gray-900 bg-opacity-40 rounded-lg">
                            ...
                            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${customCreatorInfo ? 'max-h-96 mt-6' : 'max-h-0'}`}>
                                <div className="grid grid-cols-1 md:col-span-2 gap-8">
                                    <div> <label htmlFor="creatorName" className="block text-sm font-medium text-gray-300">Creator Name</label> <input type="text" id="creatorName" value={creatorName} onChange={(e) => setCreatorName(e.target.value)} className="mt-1 block w-full bg-gray-800 border-gray-700 rounded-md shadow-sm py-3 px-4 text-white focus:outline-none focus:ring-purple-500 focus:border-purple-500" placeholder="e.g., SolMint"/> </div>
                ...
                {/* --- NEW: Result Display --- */}
                {creationResult && (
                    <div className={`mt-6 p-4 rounded-md ${creationResult.success ? 'bg-green-500 bg-opacity-20 text-green-300' : 'bg-red-500 bg-opacity-20 text-red-300'}`}>
                        <p className="font-bold">{creationResult.message}</p>
                        {creationResult.details && (
                            <pre className="mt-2 p-2 bg-black bg-opacity-20 rounded text-xs whitespace-pre-wrap font-mono">
